...
class TypeabnController extends AbstractController
{
    ...
    {
        $typeabn = new Typeabn();
        $form = $this->createForm(TypeabnType::class, $typeabn);
    ...
    #[Route('/{idtypeA}', name: 'app_typeabn_show', methods: ['GET'])]
    ...
    #[Route('/{idtypeA}/edit', name: 'app_typeabn_edit', methods: ['GET', 'POST'])]
    ...
    #[Route('/{idtypeA}', name: 'app_typeabn_delete', methods: ['POST'])]
    public function delete(Request $request, Typeabn $typeabn, TypeabnRepository $typeabnRepository): Response
    {
        if ($this->isCsrfTokenValid('delete'.$typeabn->getIdtypeA(), $request->request->get('_token'))) {
